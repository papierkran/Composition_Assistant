...
def start_processing():
    def task():
        ...
        config["OCR"]["API_KEY"] = apikey_entry.get().strip()
        config["APP"]["ROOT_DIR"] = path_entry.get().strip()
        config["DEEPSEEK"]["API_KEY"] = deepseek_entry.get().strip()
...
# OCR 配置
tk.Label(root, text="OCR 接口 URL").pack(anchor="w", padx=10)
url_entry = tk.Entry(root, width=90)
url_entry.insert(0, config["OCR"]["URL"])
url_entry.pack(padx=10)

tk.Label(root, text="APPID").pack(anchor="w", padx=10)
appid_entry = tk.Entry(root, width=90)
appid_entry.insert(0, config["OCR"]["APPID"])
appid_entry.pack(padx=10)

tk.Label(root, text="API_KEY").pack(anchor="w", padx=10)
apikey_entry = tk.Entry(root, width=90)
apikey_entry.insert(0, config["OCR"]["API_KEY"])
apikey_entry.pack(padx=10)

# DeepSeek
tk.Label(root, text="DeepSeek API Key（可选）").pack(anchor="w", padx=10, pady=(10, 0))
deepseek_entry = tk.Entry(root, width=90)
deepseek_entry.insert(0, config["DEEPSEEK"]["API_KEY"])
deepseek_entry.pack(padx=10)

use_deepseek_var = tk.BooleanVar(value=config["DEEPSEEK"]["ENABLED"])
tk.Checkbutton(
    root,
    text="启用 DeepSeek 错别字自动修正（较慢）",
    variable=use_deepseek_var
).pack(anchor="w", padx=12, pady=5)

# 路径
tk.Label(root, text="作文文件夹路径").pack(anchor="w", padx=10, pady=(10, 0))
frame = tk.Frame(root)
frame.pack(padx=10, fill="x")
path_entry = tk.Entry(frame, width=70)
path_entry.insert(0, config["APP"]["ROOT_DIR"])
path_entry.pack(side="left", fill="x", expand=True)
tk.Button(frame, text="浏览", command=browse_folder).pack(side="left", padx=8)
...
log_text = tk.Text(root, height=12, bg="#f5f5f5", state=tk.DISABLED)
